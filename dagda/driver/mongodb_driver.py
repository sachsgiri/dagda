...
class MongoDbDriver:
    ...
    def bulk_insert_cves(self, cve_list):
        ...
        for product in cve_list:
            ...
            aux = str(splitted_product[5]).strip()
            pub_date = dateutil.parser.parse(aux)
            aux = str(splitted_product[6]).strip()
            mod_date = dateutil.parser.parse(aux)
        ...
        for product in bid_list:
            ...
            try:
                if len(splitted_product) > 3:
                    date = splitted_product[3]
                    if str(date).startswith('M'):
                        mod_date = datetime.strptime(date[0:], '%b %d %Y %I:%M%p')
                        mod_date = mod_date.replace(tzinfo=timezone.utc)
                        data['mod_date'] = mod_date
                    else:
                        pub_date = datetime.strptime(date[0:], '%b %d %Y %I:%M%p')
                        pub_date = pub_date.replace(tzinfo=timezone.utc)
                        data['pub_date'] = pub_date

                if len(splitted_product) > 4:
                    date = splitted_product[4]
                    if str(date).startswith('M'):
                        mod_date = datetime.strptime(date[0:], '%b %d %Y %I:%M%p')
                        mod_date = mod_date.replace(tzinfo=timezone.utc)
                        data['mod_date'] = mod_date
